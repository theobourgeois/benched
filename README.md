...
Open the localhost URL printed by Vite. Pick the two teams and your side on the matchup screen and choose **Play game**. For a production build:
...
On the matchup screen, D-pad left/right picks your side, left stick or D-pad up/down changes that side's team, LB/RB change the mode, LT/RT change the league when more than one is available, and A or Menu starts. In other menus, the D-pad moves focus, A confirms, B goes back, and Menu resumes. In settings, left/right changes a focused selector.
